...
namespace Michsky.UI.Shift
{
    public class MainPanelManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            Invoke(nameof(EnableModalWindowsInChildren), 0.1f);
        }


        public void EnableModalWindowsInChildren()
        {
            if (panels[currentPanelIndex].panelObject.GetComponentsInChildren<Michsky.UI.Shift.ModalWindowManager>(true).Length > 0)
            {
                panels[currentPanelIndex].panelObject.GetComponentInChildren<Michsky.UI.Shift.ModalWindowManager>(true).gameObject.SetActive(true);
                    panels[currentPanelIndex].panelObject.GetComponentInChildren<Michsky.UI.Shift.ModalWindowManager>(true).ModalWindowIn();
            }
        }

        public void DisableModalWindowsInChildren()
        {
            if (panels[currentPanelIndex].panelObject.GetComponentsInChildren<Michsky.UI.Shift.ModalWindowManager>(true).Length > 0)
            {
                panels[currentPanelIndex].panelObject.GetComponentInChildren<Michsky.UI.Shift.ModalWindowManager>(true).gameObject.SetActive(false);
            }
        }
